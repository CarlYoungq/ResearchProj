...
class SETAdam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(SETAdam, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(SETAdam, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad = p.grad.data
                if grad.is_sparse:
                    raise RuntimeError('SETAdam does not support sparse gradients, please consider SparseAdam instead')
                amsgrad = group['amsgrad']
                ...
                if amsgrad:
                    ...
                    constant = torch.ones_like(p.data,
                        memory_format=torch.preserve_format) if version_higher else torch.ones_like(p.data)

                    proj_d = denom.detach().clone()
                
                    for index in range(2):
                        scalar_c = torch.sum(torch.mul(constant,proj_d))/(torch.sum(torch.pow(constant,2))+1e-20)
                        scalar_d = torch.sum(torch.mul(constant,proj_d))/(torch.sum(torch.pow(proj_d,2))+1e-20)
                        constant.mul_(scalar_c)
                        proj_d.mul_(scalar_d)

                    #exp_avg_var.mul_(beta2).add_(1 - beta2, proj_d)
                    '''
                    constant = torch.ones_like(p.data,
                        memory_format=torch.preserve_format) if version_higher else torch.ones_like(p.data)

                    proj_d = exp_avg_var.detach().clone()
                    scalar_c = torch.sum(torch.mul(constant,proj_d))/(torch.sum(torch.pow(constant,2)))
                    constant.mul_(scalar_c)                            
                    scalar_d = torch.sum(torch.mul(constant,proj_d))/(torch.sum(torch.pow(proj_d,2))+1e-20)
                    proj_d.mul_(scalar_d)
                        
                    #denom = (proj_d/bias_correction2).add(group['eps']/(1-beta2)).sqrt()
                    debias = (proj_d/bias_correction2).add(group['eps']).sqrt()
                    denom = debias.sub(0.5*torch.min(debias))
